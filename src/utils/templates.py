...
def initial_location_template(quantity: int, tropes: list, theme: str) -> str:
    ...
    user_message = (f"Create {quantity} location/s with a name and a description of the area. "
                    "The locations should connect to each other and are allowed cycles. 30% of the "
...
def flow_on_location_template(quantity: int, prev_location_name: str,
                              tropes: list, theme: str) -> str:
    """Template for generating flow on locations with a previous node as a neighbor."""
    user_message = (
        f"Create {quantity} location/s with a name and a description of the area. The locations "
        "should connect to each other and are allowed cycles. 30% of the locations should have a "
        "JSON null as a neighbour At least one Location needs to have JSON null as a neighbour. "
        f"Only one location is allowed to have a neighbour called {prev_location_name}. "
        f"{prev_location_name} Should NOT be included as a location Descriptions for the location "
        "should have no more than 100 words. Keep the location consistent with the tropes of "
        f"{tropes} and the themes of {theme}. Return this in JSON format. Following this "
        "format as an example: "
        "{‘locations’: [{ \"name\": Location, \"description\": Description of Location, "
